...
export abstract class IUserAggregatedViewRepository {
   ...
   */
  abstract findByIdentityId(identityId: string): Promise<Either<Error, UserAggregatedView | null>>;
   ...
   */
  abstract findByNationalId(nationalId: string): Promise<Either<Error, UserAggregatedView | null>>;

  /**
   * Find multiple users by criteria
   */
  abstract findByCriteria(criteria: UserProfileCriteria): Promise<Either<Error, UserAggregatedView[]>>;
