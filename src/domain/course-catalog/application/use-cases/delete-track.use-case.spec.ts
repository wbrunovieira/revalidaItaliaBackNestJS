...
describe('DeleteTrackUseCase', () => {
  ...
  describe('Track dependencies errors', () => {
    it('returns TrackHasDependenciesError when track has associated courses', async () => {
      const courseId1 = new UniqueEntityID().toString();
      const courseId2 = new UniqueEntityID().toString();
      const track = createValidTrack(undefined, [courseId1, courseId2]);
      await repo.create(track);

      // Configurar mock dependencies
      ...
      expect(result.isLeft()).toBe(true);
      if (result.isLeft()) {
        const error = result.value as TrackHasDependenciesError;
        expect(error).toBeInstanceOf(TrackHasDependenciesError);
        expect(error.message).toContain(
          'Cannot delete track because it has associated courses',
        );
        expect(error.message).toContain('JavaScript Basics');
        expect(error.message).toContain('JavaScript Advanced');
      }
    });

    it('returns TrackHasDependenciesError with single course', async () => {
      const courseId = new UniqueEntityID().toString();
      const track = createValidTrack(undefined, [courseId]);
      await repo.create(track);
      ...
      if (result.isLeft()) {
        ...
        expect(error.message).toContain('React Fundamentals');
      }
    });
    ...
    it('includes dependency info in error for frontend usage', async () => {
      ...
      repo.mockCourseDependencies.set(track.id.toString(), [
        { id: courseId, slug: 'nodejs-api', title: 'Node.js API Development' },
      ]);
      ...
      if (result.isLeft()) {
        ...
        expect(errorWithInfo.dependencyInfo.totalDependencies).toBe(1);
        expect(errorWithInfo.dependencyInfo.summary.courses).toBe(1);
      }
    });
    ...
    it('handles repository error when checking dependencies', async () => {
      ...
      vi.spyOn(repo, 'checkTrackDependencies').mockResolvedValueOnce(
  ...
  describe('Edge cases', () => {
    ...
    it('handles exception during dependencies check', async () => {
      ...
      vi.spyOn(repo, 'checkTrackDependencies').mockImplementationOnce(() => {
