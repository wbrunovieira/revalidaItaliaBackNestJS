...
describe('GetVideoUseCase', () => {
    ...
    return Video.reconstruct(
      {
        ...
        translations: [
          new VideoTranslationVO('pt', 'Título Exemplo', 'Descrição PT'),
          new VideoTranslationVO('it', 'Titolo IT', 'Descrizione IT'),
          new VideoTranslationVO('es', 'Título ES', 'Descripción ES'),
        ],
        createdAt,
        updatedAt,
      },
  ...
  it('returns video when valid ID is provided', async () => {
    ...
    const translations: Array<{
      ...
      { locale: 'pt', title: 'Título Exemplo', description: 'Descrição PT' },
  ...
  it('returns InvalidInputError when request object is missing `id`', async () => {
    ...
    if (result.isLeft() && result.value instanceof InvalidInputError) {
      expect(result.value.details.some((d) => d.path.join('.') === 'id')).toBe(
        true,
      );
    } else {
      fail('Expected InvalidInputError');
  ...
  it('returns InvalidInputError when request has extra properties', async () => {
    ...
    if (result.isLeft() && result.value instanceof InvalidInputError) {
      expect(
        result.value.details.some((d) => /unrecognized key/i.test(d.message)),
      ).toBe(true);
    } else {
