...
describe('UpdateVideoUseCase', () => {
  ...
  it('should update video details partially', async () => {
    ...
    expect(updatedVideo.isRight()).toBe(true)
    if (updatedVideo.isRight()) {
      expect(updatedVideo.value.video.imageUrl).toBe('https://example.com/new.jpg')
  ...
  it('should update video translations', async () => {
    ...
    const translations = [
      { locale: 'pt' as const, title: 'Título Antigo', description: 'Descrição Antiga' },
      { locale: 'it' as const, title: 'Titolo Vecchio', description: 'Descrizione Vecchia' },
      { locale: 'es' as const, title: 'Título Viejo', description: 'Descripción Vieja' },
    ]
    await videoRepository.create('lesson-123', video, translations)
    ...
    const result = await sut.execute({
      ...
      translations: [
        ...
        { locale: 'it', title: 'Nuovo Titolo', description: 'Nuova Descrizione' },
        { locale: 'es', title: 'Nuevo Título', description: 'Nueva Descripción' },
      ],
    })
    ...
    if (updatedVideo.isRight()) {
      ...
      expect(translations?.find((t) => t.locale === 'pt')?.title).toBe('Novo Título')
      expect(translations?.find((t) => t.locale === 'it')?.title).toBe('Nuovo Titolo')
      expect(translations?.find((t) => t.locale === 'es')?.title).toBe('Nuevo Título')
    }
  })
  ...
  it('should update multiple fields at once', async () => {
    ...
    if (updatedVideo.isRight()) {
      ...
      expect(updatedVideo.value.video.lessonId).toBe(lesson.id.toString())
      expect(
        updatedVideo.value.video.translations.find((t) => t.locale === 'pt')?.title,
      ).toBe('Novo Título')
    }
  })
  ...
  it('should return error when new slug is already taken by another video', async () => {
    ...
    const translations1 = [
      ...
      { locale: 'it' as const, title: 'Titolo 1', description: 'Descrizione 1' },
      { locale: 'es' as const, title: 'Título 1', description: 'Descripción 1' },
    ]
    ...
    const translations2 = [
      ...
      { locale: 'it' as const, title: 'Titolo 2', description: 'Descrizione 2' },
      { locale: 'es' as const, title: 'Título 2', description: 'Descripción 2' },
    ]
