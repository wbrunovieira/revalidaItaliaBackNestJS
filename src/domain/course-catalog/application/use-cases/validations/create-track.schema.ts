  ...
  translations: z.array(translationSchema)
    ...
    .superRefine((arr, ctx) => {
      if (!arr.some(t => t.locale === "pt")) {
        ctx.addIssue({ code: z.ZodIssueCode.custom, message: "At least a Portuguese translation is required", path: ["translations"] });
      }
      const locales = arr.map(t => t.locale);
      if (new Set(locales).size !== locales.length) {
        ctx.addIssue({ code: z.ZodIssueCode.custom, message: "Duplicate locale in translations", path: ["translations"] });
      }
    }),
});
