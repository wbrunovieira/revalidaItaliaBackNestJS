  ...
  url: z.string().min(1, 'URL is required').refine(
    (url) => {
      ...
      try {
        ...
        return /^[\/\\]?[\w\-\._\/\\]+\.\w+$/.test(url) || /^\.?[\/\\][\w\-\._\/\\]+\.\w+$/.test(url);
      }
    },
    {
...
export const updateDocumentSchema = z.object({
  ...
  fileSize: z.number().int().min(1, 'File size must be greater than 0').max(100 * 1024 * 1024, 'File size cannot exceed 100MB').optional(),
