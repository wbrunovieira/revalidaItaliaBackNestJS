...
describe('QuestionController - GetQuestion', () => {
  ...
  describe('Success scenarios', () => {
    it('should return question for valid ID', async () => {
      const requestData = QuestionControllerTestData.getQuestion.validIds.standard();
      ...
      const result = await helpers.executeGetByIdExpectSuccess(requestData.id, expectedQuestion);

      helpers.verifyGetQuestionResponseStructure(result, requestData.id, expectedQuestion);
    });

    it('should return multiple choice question with all fields', async () => {
      const requestData = QuestionControllerTestData.getQuestion.validIds.multipleChoice();
      const expectedQuestion = {
        id: requestData.id,
    ...
    it('should return open question with argument', async () => {
      const requestData = QuestionControllerTestData.getQuestion.validIds.openQuestion();
      const expectedQuestion = {
        id: requestData.id,
    ...
    it('should return question without argument', async () => {
      const requestData = QuestionControllerTestData.getQuestion.validIds.withoutArgument();
      const expectedQuestion = {
        id: requestData.id,
    ...
    it('should handle mixed case UUID correctly', async () => {
      ...
      const result = await helpers.executeGetByIdExpectSuccess(mixedCaseId, expectedQuestion);

      helpers.verifyGetQuestionResponseStructure(result, mixedCaseId, expectedQuestion);
    });

    it('should return question with special characters in text', async () => {
      const requestData = QuestionControllerTestData.getQuestion.validIds.edgeCase();
      const expectedQuestion = {
        id: requestData.id,
  ...
  describe('Validation and Invalid Input scenarios', () => {
    it('should reject invalid UUID format', async () => {
      const invalidId = QuestionControllerTestData.getQuestion.invalidIds.invalidFormat().id;
      
      await helpers.executeGetByIdExpectBadRequest(invalidId);
    });

    it('should reject non-UUID string', async () => {
      const invalidId = QuestionControllerTestData.getQuestion.invalidIds.notUuid().id;
      
      await helpers.executeGetByIdExpectBadRequest(invalidId);
    });

    it('should reject too short ID', async () => {
      const invalidId = QuestionControllerTestData.getQuestion.invalidIds.tooShort().id;
      
      await helpers.executeGetByIdExpectBadRequest(invalidId);
    });

    it('should reject too long ID', async () => {
      const invalidId = QuestionControllerTestData.getQuestion.invalidIds.tooLong().id;
      
      await helpers.executeGetByIdExpectBadRequest(invalidId);
    });

    it('should reject ID with wrong hyphen placement', async () => {
      const invalidId = QuestionControllerTestData.getQuestion.invalidIds.wrongHyphens().id;
      
      await helpers.executeGetByIdExpectBadRequest(invalidId);
    });

    it('should reject ID missing hyphens', async () => {
      const invalidId = QuestionControllerTestData.getQuestion.invalidIds.missingHyphens().id;
      
      await helpers.executeGetByIdExpectBadRequest(invalidId);
    });

    it('should reject ID with special characters', async () => {
      const invalidId = QuestionControllerTestData.getQuestion.invalidIds.specialChars().id;
      
      await helpers.executeGetByIdExpectBadRequest(invalidId);
    });

    it('should reject ID with invalid hex characters', async () => {
      const invalidId = QuestionControllerTestData.getQuestion.invalidIds.invalidChars().id;
      
      await helpers.executeGetByIdExpectBadRequest(invalidId);
    });

    it('should reject empty string ID', async () => {
      const invalidId = QuestionControllerTestData.getQuestion.invalidIds.emptyString().id;
      
      await helpers.executeGetByIdExpectBadRequest(invalidId);
    });

    it('should reject ID with whitespace', async () => {
      const invalidId = QuestionControllerTestData.getQuestion.invalidIds.withWhitespace().id;
      
      await helpers.executeGetByIdExpectBadRequest(invalidId);
    });

    it('should reject ID with tabs', async () => {
      const invalidId = QuestionControllerTestData.getQuestion.invalidIds.withTabs().id;
      
      await helpers.executeGetByIdExpectBadRequest(invalidId);
    });

    it('should reject ID with newlines', async () => {
      const invalidId = QuestionControllerTestData.getQuestion.invalidIds.withNewlines().id;
      
      await helpers.executeGetByIdExpectBadRequest(invalidId);
    });

    it('should reject ID with unicode characters', async () => {
      const invalidId = QuestionControllerTestData.getQuestion.invalidIds.unicodeChars().id;
      
      await helpers.executeGetByIdExpectBadRequest(invalidId);
    });

    it('should reject ID with emojis', async () => {
      const invalidId = QuestionControllerTestData.getQuestion.invalidIds.emojis().id;
  ...
  describe('Edge cases', () => {
    it('should handle all zeros UUID', async () => {
      const zerosId = QuestionControllerTestData.getQuestion.nonExistentIds.zeros().id;
  ...
  describe('Error scenarios', () => {
    it('should throw NotFoundException when question does not exist', async () => {
      const nonExistentId = QuestionControllerTestData.getQuestion.nonExistentIds.notFound().id;
      
      await helpers.executeGetByIdExpectNotFound(nonExistentId);
    });

    it('should throw NotFoundException for deleted question', async () => {
      const deletedId = QuestionControllerTestData.getQuestion.nonExistentIds.deleted().id;
      
      await helpers.executeGetByIdExpectNotFound(deletedId);
    });

    it('should throw InternalServerErrorException on repository error', async () => {
      const validId = QuestionControllerTestData.getQuestion.validIds.standard().id;
    ...
    it('should handle repository timeout error', async () => {
      ...
      await expect(testSetup.controller.getById(validId)).rejects.toThrow(InternalServerErrorException);
      
      expect(testSetup.getUseCase.execute).toHaveBeenCalledWith({ id: validId });
    });
    ...
    it('should handle validation error with multiple details', async () => {
      ...
      await expect(testSetup.controller.getById(invalidId)).rejects.toThrow(BadRequestException);
      
      expect(testSetup.getUseCase.execute).toHaveBeenCalledWith({ id: invalidId });
    });
  });

  describe('Request format validation', () => {
    it('should handle number as ID parameter', async () => {
      const numberId = QuestionControllerTestData.getQuestion.invalidIds.numberValue().id;
    ...
    it('should handle boolean as ID parameter', async () => {
      const booleanId = QuestionControllerTestData.getQuestion.invalidIds.booleanValue().id;
  ...
  describe('Performance tests', () => {
    it('should execute getById within acceptable time', async () => {
      ...
      const { executionTime } = await helpers.testGetQuestionPerformance(validId, 100);
    ...
    it('should handle concurrent requests efficiently', async () => {
      ...
      const { executionTime } = await helpers.testConcurrentGetQuestionRequests(concurrentIds, 500);
  ...
  describe('Behavioral validation', () => {
    it('should call use case with exact ID parameter', async () => {
      const testId = QuestionControllerTestData.getQuestion.validIds.standard().id;
    ...
    it('should preserve ID case sensitivity', async () => {
      ...
      expect(testSetup.getUseCase.execute).toHaveBeenCalledWith({ id: mixedCaseId });
    });

    it('should not modify request data', async () => {
      const originalId = QuestionControllerTestData.getQuestion.validIds.standard().id;
    ...
    it('should return consistent response structure', async () => {
      ...
      expect(Object.keys(result1.question)).toEqual(Object.keys(result2.question));
    });
