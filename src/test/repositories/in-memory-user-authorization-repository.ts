...
export class InMemoryUserAuthorizationRepository extends IUserAuthorizationRepository {
  ...
  async save(
    ...
    const index = this.items.findIndex(
      (item) =>
        item.identityId.toString() === userAuthorization.identityId.toString(),
    );
  ...
  async findByIdentityId(
    ...
    const userAuthorization = this.items.find(
  ...
  async findActive(): Promise<Either<Error, UserAuthorization[]>> {
    ...
    const activeAuthorizations = this.items.filter((item) => {
      const isAfterEffectiveFrom =
        !item.effectiveFrom || item.effectiveFrom <= now;
      const isBeforeEffectiveUntil =
  ...
  async findExpired(): Promise<Either<Error, UserAuthorization[]>> {
    const now = new Date();
    const expiredAuthorizations = this.items.filter(
      (item) => item.effectiveUntil && item.effectiveUntil <= now,
    );
