...
export class InMemoryUserProfileRepository extends IUserProfileRepository {
  ...
  async save(userProfile: UserProfile): Promise<Either<Error, void>> {
    const index = this.items.findIndex(
      (item) =>
        item.identityId.toString() === userProfile.identityId.toString(),
    );
  ...
  async findByIdentityId(
    ...
    const userProfile = this.items.find(
  ...
  async findByNationalId(
    ...
    const userProfile = this.items.find((item) =>
      item.nationalId.equals(nationalId),
    );
    return right(userProfile || null);
  }
