...
export class AssessmentTestHelpers {
  ...
  async getQuestionsDetailed(assessmentId: string): Promise<request.Response> {
    return request(this.app.getHttpServer())
      .get(`/assessments/${assessmentId}/questions/detailed`);
  }
  ...
  validateQuizResponse(response: any, expectedData: any) {
    ...
    expect(response.body.questions[0].answer.correctOptionId).toBe(expectedData.option2Id);
    expect(response.body.questions[0].answer.explanation).toBe('Two plus two equals four');
  ...
  validateSimuladoResponse(response: any, expectedData: any) {
    ...
    // Validate questions
    expect(response.body.questions).toHaveLength(1);
    expect(response.body.questions[0].text).toBe('What is the normal heart rate?');
    expect(response.body.questions[0].argumentId).toBe(expectedData.argumentId);
    
    // Validate answer
    expect(response.body.questions[0].answer).toBeDefined();
    expect(response.body.questions[0].answer.correctOptionId).toBe(expectedData.simOption2Id);
  }
  ...
  validateProvaAbertaResponse(response: any, expectedData: any) {
    ...
    // Validate questions
    expect(response.body.questions).toHaveLength(1);
    expect(response.body.questions[0].text).toBe('Explain the cardiovascular system');
