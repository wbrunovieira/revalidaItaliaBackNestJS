...
describe('Create Module (E2E)', () => {
  ...
  beforeAll(async () => {
    ...
    const courseRes = await request(app.getHttpServer())
      ...
      .send({
        slug: 'curso-pai',
        translations: [
          { locale: 'pt', title: 'Curso Pai', description: 'Descrição Curso Pai' },
          { locale: 'it', title: 'Corso Padre', description: 'Descrizione Corso Padre' },
          { locale: 'es', title: 'Curso Padre', description: 'Descripción Curso Padre' },
        ],
      });
  ...
  it('[POST] /courses/:courseId/modules - Success', async () => {
    const payload = {
      slug: 'modulo-1',
      translations: [
        { locale: 'pt', title: 'Módulo Um', description: 'Descrição Módulo Um' },
        { locale: 'it', title: 'Modulo Uno', description: 'Descrizione Modulo Uno' },
        { locale: 'es', title: 'Módulo Uno', description: 'Descripción Módulo Uno' },
      ],
      order: 1,
    };
  ...
  it('[POST] /courses/:courseId/modules - Missing Portuguese Translation', async () => {
    const payload = {
      slug: 'modulo-sem-pt',
      translations: [
        { locale: 'it', title: 'Modulo Due', description: 'Descrizione Modulo Due' },
        { locale: 'es', title: 'Módulo Dos', description: 'Descripción Módulo Dos' },
      ],
      order: 2,
    };
  ...
  it('[POST] /courses/:courseId/modules - Duplicate Module Order', async () => {
    ...
    await request(app.getHttpServer())
      ...
      .send({
        ...
        translations: [
          ...
          { locale: 'es', title: 'Módulo Tres', description: 'Descripción Tres' },
        ],
        order: 3,
      });
    ...
    const res = await request(app.getHttpServer())
      ...
      .send({
        slug: 'modulo-3b',
        translations: [
          { locale: 'pt', title: 'Módulo Três B', description: 'Descrição Três B' },
          { locale: 'it', title: 'Modulo Tre B', description: 'Descrizione Tre B' },
          { locale: 'es', title: 'Módulo Tres B', description: 'Descripción Tres B' },
        ],
        order: 3,
      });
