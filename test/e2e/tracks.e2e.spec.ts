  ...
  it('[POST] same course in multiple tracks - Success', async () => {
    const courseRes = await request(app.getHttpServer())
      ...
      .send({
        ...
        translations: [
          { locale: 'pt', title: 'Curso PT Multi', description: 'Desc PT Multi' },
          { locale: 'it', title: 'Corso IT Multi', description: 'Desc IT Multi' },
          { locale: 'es', title: 'Curso ES Multi', description: 'Desc ES Multi' },
        ],
      });
    expect(courseRes.status).toBe(201);
    const courseId = courseRes.body.id;
    ...
    const track1 = await request(app.getHttpServer())
      ...
      .send({
        ...
        courseIds: [courseId],
        translations: [
          { locale: 'pt', title: 'Trilha 1 PT', description: 'Desc Trilha1 PT' },
          { locale: 'it', title: 'Traccia 1 IT', description: 'Desc Traccia1 IT' },
    ...
    const track2 = await request(app.getHttpServer())
      ...
      .send({
        ...
        courseIds: [courseId],
        translations: [
          { locale: 'pt', title: 'Trilha 2 PT', description: 'Desc Trilha2 PT' },
          { locale: 'it', title: 'Traccia 2 IT', description: 'Desc Traccia2 IT' },
    ...
    expect(slugs).toEqual(expect.arrayContaining(['trilha-multi-1', 'trilha-multi-2']));
  });
});